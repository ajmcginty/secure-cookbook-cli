import json
import os
from utils.encryption import verify_password, generate_user_key
...
def add_recipe(username, recipe_name, recipe_entry):
    """
    Adds recipe input to JSON.
    """
    data = load_data()
    ...
    # Call an encrypt function here on the data name_encrypted

    user["recipes"][recipe_name] = recipe_entry
    save_data(data)

def view_recipe(username, recipe_name):
    """
    View a specific recipe for a given user.
    """
    data = load_data()
    user = data["users"].get(username)
